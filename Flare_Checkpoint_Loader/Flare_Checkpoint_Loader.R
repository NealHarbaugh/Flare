...
loss_leaderboard_path <- "~/path/to/top_losses.csv"

Attempted_hyperperameters <- "~/path/to/hyperparameter_combinations_used.csv"
...
###-------Recover completed processes-------------------------------------------

# Load the leaderboard file with loss values and their corresponding epoch and 
# batch size values
leaderboard <- read.csv(loss_leaderboard_path)

# Initialize variables to store best hyperparameters and their performance
best_epochs <- numeric(3)
best_batch_size <- numeric(3)
best_losses <- numeric(3)

# Store the values for loss, batch size, and training epochs
best_batch_size[1] <- leaderboard$Batch_Size[1]
best_batch_size[2] <- leaderboard$Batch_Size[2]
best_batch_size[3] <- leaderboard$Batch_Size[3]

best_epochs[1] <- leaderboard$Training_Epochs[1]
best_epochs[2] <- leaderboard$Training_Epochs[2]
best_epochs[3] <- leaderboard$Training_Epochs[3]

best_losses[1] <- leaderboard$Loss[1]
best_losses[2] <- leaderboard$Loss[2]
best_losses[3] <- leaderboard$Loss[3]

First_best_loss <- leaderboard$Loss[1]
Second_best_loss <- leaderboard$Loss[2]
Third_best_loss <- leaderboard$Loss[3]

# Load the file with hyperparameters used
hyperparameters <- read.csv(Attempted_hyperperameters)

# Remove any extra columns that are not Training_Epochs or Batch_Size
hyperparameters <- hyperparameters[, c("batch_size", "training_epochs")]

# Find the last Batch_Size and Training_Epochs values that were used
last_row <- NULL
for (i in nrow(hyperparameters):1) {
  if (any(hyperparameters[i, ] != 0 & !is.na(hyperparameters[i, ]))) {
    last_row <- hyperparameters[i, ]
    break
  }
}
...
# Generate all possible combinations of epochs and batch sizes
...
hyperparameters_df <- data.frame(
  batch_size = numeric(nrow(hyperparameter_combinations)),
  training_epochs = numeric(nrow(hyperparameter_combinations))
)

# Remove rows in hyperparameters that have a 0 in either batch_size or 
# training_epochs
hyperparameters_processed <- hyperparameters

hyperparameters_processed <- 
  hyperparameters_processed[!(hyperparameters_processed$batch_size == 0 | 
                              hyperparameters_processed$training_epochs == 0), ]

if (!is.null(last_row)) {
  # Get the last used values of Batch_Size and Training_Epochs
  last_batch_size <- last_row$batch_size
  last_training_epochs <- last_row$training_epochs
  
  # Number of rows to replace
  n <- min(nrow(hyperparameters_df), nrow(hyperparameters_processed))
  
  # Replace the first n rows of hyperparameters_df with 
  # Partial_hyperparameter_combinations
  hyperparameters_df[1:n, ] <- hyperparameters_processed[1:n, ]
  
} else {
  print("No row with non-zero and non-NA values found.")
}

# Remove rows that have a 0 in either batch_size or training_epochs
hyperparameters <- 
  hyperparameters[!(hyperparameters$batch_size == 0 | 
                      hyperparameters$training_epochs == 0), ]

model_path <- paste0(model_save_path, "/Flare_model.h5")

Validation_path <- paste0(model_save_path, "/Temporary_folder/Validation.csv")
Validation_df <- read.csv(Validation_path)

Codex_path <- paste0(model_save_path, "/Temporary_folder/Codex.csv")
...
postburn_data <- postburn_data[-18, ] # Delete this later
...
for (col in 1:length(columns_with_letters))
{
  ...
  preburn_data_processed <- reencode_column(preburn_data_processed, Codex_df, 
                                            col_name)
  postburn_data_processed <- reencode_column(postburn_data_processed, Codex_df, 
                                             col_name)
}
...
validation_indices_preburn <- Validation_df$Index
...
# Set the path to each of the 3 folders
Folder_1 <- paste0(model_save_path, "/1") 
Folder_2 <- paste0(model_save_path, "/2")
Folder_3 <- paste0(model_save_path, "/3")

Ignis_model_path <- paste0(Folder_1, "/Ignis_model.h5")
Ignis_model <- load_model_hdf5(model_path, compile = TRUE)
Pyra_model_path <- paste0(Folder_2, "/Pyra_model.h5")
Pyra_model <- load_model_hdf5(model_path, compile = TRUE)
Scorch_model_path <- paste0(Folder_3, "/Scorch_model.h5")
Scorch_model <- load_model_hdf5(model_path, compile = TRUE)
...
Testing_iterations_done <- nrow(hyperparameters)
Testing_iterations <- nrow(hyperparameter_combinations)
Testing_iterations_different <- Testing_iterations - Testing_iterations_done

# Create the progress bar
pb <- progress_bar$new(
  total = Testing_iterations_different,
  format = paste(":current/:total [:bar] ETA: :eta ", sep = ""), 
  clear = FALSE)

# Perform the grid search
for (g in Testing_iterations_done:Testing_iterations)
{
  ...
  for (epoch in 1:epochs) {
    ...
    if (epoch < epochs) {
      ...
      # Make sure end_index does not exceed the total number of samples
      end_index <- min(end_index, nrow(postburn_data_processed))
    } else if (epoch == epochs) {
      total_samples <- nrow(postburn_data_processed)
      full_batches <- ceiling(total_samples / batch_size) - 1
      remaining_samples <- total_samples - (full_batches * batch_size)
  ...
  trained_validation_preburn_matrix <- as.matrix(trained_validation_preburn)
  trained_validation_postburn_matrix <- as.matrix(trained_validation_postburn)
  ...
  if (validation_loss < First_best_loss) {
    ...
    # Save batch_size and training_epochs to Folder_1
    write.csv(hyperparameters_df, file.path(Folder_1, "hyperparameters.csv"), 
              row.names = FALSE)
    ...
    # Save batch_size and training_epochs to Folder_1
    write.csv(hyperparameters_df, file.path(Folder_2, "hyperparameters.csv"), 
              row.names = FALSE)
    ...
    write.csv(hyperparameters_df, file.path(Folder_3, "hyperparameters.csv"), 
              row.names = FALSE)
  ...
  hyperparameters_df[row_to_update, "training_epochs"] <- training_epochs
  
  write.csv(hyperparameters_df, 
